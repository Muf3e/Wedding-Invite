 ...
 *   - the seal-tap glow waves are pure CSS (lavender.css) driven by app.js's
...
(function () {
  ...
  function flowerSpot() {
    // hover beside the roses in the corners of the arch artwork when they are on screen
    var spots = [];
    var art = document.querySelector('#hero-archway-stage .vx-arch-img');
    if (art) {
      var r = art.getBoundingClientRect();
      var fr = [[0.17, 0.10], [0.83, 0.10], [0.13, 0.87], [0.87, 0.87]];
      for (var i = 0; i < fr.length; i++) {
        var x = r.left + r.width * fr[i][0], y = r.top + r.height * fr[i][1];
        if (r.width > 0 && y > 60 && y < vh() - 60) spots.push({ x: x, y: y });
      }
    }
  ...
  function startWildlife(delayMs) {
    ...
    setTimeout(function () {
      ...
      // inside the invitation (it isolates its own stacking context) so the layer sits BEHIND the text
